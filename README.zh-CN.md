...
5. **写入前先在内存中验证**，随后**原子写入**文件，再从磁盘做**往返校验**——因此绝不会留下
   半成品或无效镜像。
...
python3 patch_ae.py FWHC940A.bin --ir-offset 0x6cb628                 # 明确选择某一张表
python3 patch_ae.py FWHC940A.bin --all               # 若存在多张表，全部打补丁
python3 patch_ae.py FWHC940A.bin --version           # 打印工具版本
```
...
- **“N tab_ratio_ir candidates”（多个候选）或改到了错误的表** —— 用 `--ir-offset 0x<偏移>` 指定
  一张（偏移会打印出来），或用 `--all` 给所有表打补丁。
- **“checksum self-test failed …”** —— 较新的容器变体；用 `NTKFWinfo -i` 找到正确偏移并传
  `--uit-off/--uit-size`。
- **“could not auto-detect uITRON”** —— 用 `NTKFWinfo -i` 的偏移传 `--uit-off/--uit-size`。
- **“original file CRC mismatch”** —— 输入疑似已被修改；请重新获取干净固件（或在知情风险时加 `--force`）。
- 只有**两个校验和都通过**才会写文件；任何失败都会以非零码退出且不写任何文件。
